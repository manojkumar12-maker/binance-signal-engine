...
def _mem_expired(signal: Dict) -> bool:
    locked_at = signal.get("locked_at", 0)
    decay = signal.get("ttl", 1800)
    return time.time() - locked_at > decay
...
def acquire_lock(symbol: str, ttl: int = 10) -> bool:
    ...
    now = time.time()
    expiry = _MEMORY_LOCKS.get(symbol, 0)
    if expiry > now:
        return False
    _MEMORY_LOCKS[symbol] = now + ttl
    return True
...
def store_signal(signal: Dict, state: str = "PENDING") -> Optional[Dict]:
    ...
    locked_signal["locked_timestamp"] = datetime.now(timezone.utc).isoformat()
    ...
    if r:
        ...
        _MEMORY_SIGNALS[pair] = json.dumps(locked_signal)
...
def get_stored_signal(pair: str) -> Optional[Dict]:
    ...
    raw = _MEMORY_SIGNALS.get(pair)
    if not raw:
        return None
    signal = json.loads(raw)
    if _mem_expired(signal):
        _MEMORY_SIGNALS.pop(pair, None)
        return None
    return signal
...
def update_signal_state(pair: str, new_state: str, reason: str = "") -> bool:
    ...
    if reason:
        signal["state_reason"] = reason
    _MEMORY_SIGNALS[pair] = json.dumps(signal)
    return True
...
def get_all_stored_signals(state: Optional[str] = None) -> List[Dict]:
    ...
    signals: List[Dict] = []
    expired_keys = []
    for pair, raw in list(_MEMORY_SIGNALS.items()):
        signal = json.loads(raw)
        if _mem_expired(signal):
            expired_keys.append(pair)
            continue
        if state is None or signal.get("signal_state") == state:
            signals.append(signal)
...
def clear_expired_signals() -> int:
    ...
    for pair, raw in list(_MEMORY_SIGNALS.items()):
        signal = json.loads(raw)
        state = signal.get("signal_state")
        if state in [SIGNAL_STATES["EXPIRED"], SIGNAL_STATES["REJECTED"], SIGNAL_STATES["CLOSED"]] or _mem_expired(signal):
            _MEMORY_SIGNALS.pop(pair, None)
            cleared += 1
    return cleared
